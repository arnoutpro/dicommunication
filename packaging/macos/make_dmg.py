...
APP_BUNDLE_NAMES = ("Dicommunication.app",)
README_NAME = "Read Me.txt"

README_TEXT = """Dicommunication — Arnout.pro

1. Drag Dicommunication.app onto Applications, or straight onto your Desktop
   for a quick-launch icon.
2. The first time, right-click the app and choose Open (the build is unsigned;
   Gatekeeper will warn).
3. It opens in its own window (not a browser tab, though
   http://127.0.0.1:8080/ works too if you'd rather use one). Dicomtag
   Analytics, Dicom Anonymizer, Dicom Router, and Dicom Cleaner are all
   reachable from this one window's own navigation.
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        nargs="+",
        help="Path(s) to Dicommunication.app",
    )
