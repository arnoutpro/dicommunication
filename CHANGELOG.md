...
- **Tag Editor**: new tool. Corrects `Tamar Report Final Sign Timestamp` (07a3,10fb) and `Tamar Report Study Last Composed By` (07a5,1040) — private ELSCINT1 tags CS PACS REPORT writes onto every instance in a study — for reports stuck because one of them didn't get set, then C-STOREs the fix back. **Fetch** C-MOVEs the study (or one series) and shows current values per instance; **Push** overwrites only the field(s) given and re-stores every retrieved instance, since Vue mirrors both tags across the whole study. Only ever overwrites a tag that is already present — never invents one on an instance that never had it. The local Storage SCP now also accepts common image SOP Classes (CT, MR, DX, CR, US, Secondary Capture, NM, PET, XA, RF, mammography) alongside Structured Reports, so this and future retrieve-based tools aren't limited to SR.
